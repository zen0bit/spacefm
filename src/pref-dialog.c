...

{
    ...
    if (response == GTK_RESPONSE_OK)
    {
        ...
        if (g_strcmp0(terminal, old_terminal))
        {
            ...
            root_set_change = TRUE;
            if (terminal && (strstr(terminal, "lxterminal") || strstr(terminal, "urxvtc") ||
            ...
            {
                str = g_strdup_printf(
                    _("Use of some terminals (%s) with SpaceFM may prevent the Run As Task and Run "
                      ...
                      "terminal to fail to automatically open after being mounted.  For full use "
                      "of SpaceFM's features, selecting another terminal is recommended."),
