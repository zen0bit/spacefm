 ...
 * Since: 0.3.1.3
 **/
GdkPixbuf* exo_gdk_pixbuf_lucent(const GdkPixbuf* source, unsigned int percent)
{
    GdkPixbuf* dst;
    ...
    g_return_val_if_fail(GDK_IS_PIXBUF(source), NULL);
    g_return_val_if_fail((int)percent >= 0 && percent <= 100, NULL);
    ...
    if (G_LIKELY(gdk_pixbuf_get_has_alpha(source)))
    {
        for (i = height; --i >= 0;)
        {
            ...
            for (j = width; --j >= 0;)
            {
                ...
                *pixdst++ = *pixsrc++;
                *pixdst++ = *pixsrc++;
                *pixdst++ = ((unsigned int)*pixsrc++ * percent) / 100u;
            }
        }
    }
    else
 ...
 **/
GdkPixbuf* exo_gdk_pixbuf_scale_ratio(GdkPixbuf* source, int dest_size)
{
    double wratio;
    double hratio;
    int source_width;
    ...
    wratio = (double)source_width / (double)dest_size;
    hratio = (double)source_height / (double)dest_size;
...
static void size_prepared(GdkPixbufLoader* loader, int width, int height, SizePreparedInfo* info)
{
    bool scalable;
    double wratio;
    double hratio;
    ...
    if (scalable || width > info->max_width || height > info->max_height)
    {
        ...
        {
            /* calculate the new dimensions */
            wratio = (double)width / (double)info->max_width;
            hratio = (double)height / (double)info->max_height;
 ...
 * Since: 0.3.1.9
 **/
GdkPixbuf* exo_gdk_pixbuf_new_from_file_at_max_size(const char* filename, int max_width,
                                                    int max_height, bool preserve_aspect_ratio,
                                                    GError** error)
{
    ...
    err1: /* initialize the library's i18n support */
        ...
        g_set_error(error,
                    G_FILE_ERROR,
                    g_file_error_from_errno(sverrno),
...
#endif
    {
        ...
        buffer = g_newa(unsigned char, 8192);
