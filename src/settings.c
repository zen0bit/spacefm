...
void load_settings(char* config_dir)
{
    ...
    if (ver < 26) // < hand  (1.0.0 alpha)
    {
        ...
        if (set && set->s && set->s[0])
        {
            ...
            cmd = g_strdup_printf("# Imported protocol handler from "
                                  "SpaceFM 0.9:\n\n%s %%url%%",
                                  set->s);
            str = ptk_handler_save_script(HANDLER_MODE_NET, HANDLER_MOUNT, handset, NULL, cmd);
            ...
            str = ptk_handler_save_script(HANDLER_MODE_NET, HANDLER_UNMOUNT, handset, NULL, cmd);
            g_free(str); // ignore any error msg
            g_free(cmd);

            // set a properties command to handler script
            cmd = g_strdup("mount | grep \"%a\"");
            str = ptk_handler_save_script(HANDLER_MODE_NET, HANDLER_PROP, handset, NULL, cmd);
            g_free(str); // ignore any error msg
            g_free(cmd);
        ...
        str = ptk_handler_save_script(HANDLER_MODE_FS, HANDLER_UNMOUNT, handset, NULL, cmd);
        g_free(str); // ignore any error msg
        g_free(cmd);
        ...
        str = ptk_handler_save_script(HANDLER_MODE_FS, HANDLER_MOUNT, handset, NULL, cmd);
        g_free(str); // ignore any error msg
        g_free(cmd);
        // Copy default Properties command
        str = ptk_handler_save_script(HANDLER_MODE_FS, HANDLER_LIST, handset, NULL, INFO_EXAMPLE);
    ...
    if (ver < 32 && !xset_is("panel1_tool_l") /*only once*/) // < 1.0.2
    {
        ...
        XSet* new_set;
        int j, p;
        const char* old_toolbar_list[] = {"tool_device",
                                          "tool_book",
                                          "tool_dirtree",
                                          "tool_newtab",
                                          "tool_newtabhere",
                                          ...
                                          "tool_default",
                                          "tool_refresh"};
        char new_toolbar_types[] = {XSET_TOOL_DEVICES,
                                    XSET_TOOL_BOOKMARKS,
                                    XSET_TOOL_TREE,
                                    XSET_TOOL_NEW_TAB,
            ...
            for (j = 0; j < 3; j++) // left, right, and side
            {
                ...
                for (i = 0; i < G_N_ELEMENTS(old_toolbar_list); i++)
                {
                    ...
                    if (j == 0)
                        ...
                        name = g_strdup_printf("%c%s", ch, old_toolbar_list[i]);
                    ...
                    if (old_set->tool == XSET_B_TRUE)
                    {
                        ...
                        new_set = xset_new_builtin_toolitem(new_toolbar_types[i]);
                        if (!child_set)
                        {
                        ...
                        // copy custom icon
                        if (old_set->icon && old_set->keep_terminal == XSET_B_TRUE)
                        {
        ...
        // remove xsets
        const char* old_toolbar_sets[] = {"toolbar_left",
                                          "toolbar_side",
                                          "toolbar_right",
                                          "toolbar_hide",
                                          "toolbar_hide_side",
...
char* save_settings(gpointer main_window_ptr)
{
                ...
                if (pages) // panel was shown
                {
                    ...
                    for (g = 0; g < pages; g++)
                    {
                        file_browser = PTK_FILE_BROWSER(
                            gtk_notebook_get_nth_page(GTK_NOTEBOOK(main_window->panel[p - 1]), g));
                        old_tabs = tabs;
    ...
    {
        /* General */
        result = fputs(_("# SpaceFM Session File\n\n# THIS FILE IS NOT DESIGNED TO BE EDITED - it "
                         "will be read and OVERWRITTEN\n\n# If you delete all session* files, "
                         "SpaceFM will be reset to factory defaults.\n\n"),
                       file);
        if (result < 0)
            goto _save_error;
        fputs("[General]\n", file);
...
void write_root_saver(FILE* file, const char* path, const char* name, const char* var,
                      const char* value)
{
    if (!value)
        return;
...
gboolean write_root_settings(FILE* file, const char* path)
{
    ...
    fprintf(file,
            "\n# save root settings\nmkdir -p %s/spacefm\necho -e '# SpaceFM As-Root Session "
                         ...
                         const char* plug_dir, int type, int job, XSet* insert_set)
{
    ...
    if (job == PLUGIN_JOB_INSTALL)
    {
        // install
        own =
            g_strdup_printf("chown -R root:root %s && chmod -R go+rX-w %s", plug_dir_q, plug_dir_q);
        task->task->exec_as_user = g_strdup("root");
    }
    else
    {
...
void xset_custom_export(GtkWidget* parent, PtkFileBrowser* file_browser, XSet* set)
{
    ...
    if (!set->plugin)
        task->task->exec_command =
            g_strdup_printf("tar --numeric-owner -czf %s * ; err=$? ; rm -rf %s ; if [ $err -ne 0 "
                            "]; then rm -f %s; fi; exit $err",
                            path_q,
                            plug_dir_q,
                            path_q);
    else
        task->task->exec_command = g_strdup_printf("tar --numeric-owner -czf %s * ; err=$? ; if [ "
        ...
        case XSET_JOB_COMMAND:
            ...
            else if (job == XSET_JOB_APP)
            {
                VFSMimeType* mime_type =
                    vfs_mime_type_get_from_type(xset_context && xset_context->var[CONTEXT_MIME] &&
