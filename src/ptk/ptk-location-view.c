                                     ...
                                     gboolean new_tab, gboolean force_new_mount)
{
    ...
    */
    ...
    char* keepterm;
    if (ssh_udevil)
        keepterm = g_strdup_printf(
            "if [ $? -ne 0 ]; then\n    echo \"%s\"\n    read\nelse\n    echo; echo \"Press Enter "
            "to close (closing this window may unmount sshfs)\"\n    read\nfi\n",
            press_enter_to_close);
    else if (run_in_terminal)
        keepterm =
            g_strdup_printf("[[ $? -eq 0 ]] || ( echo -n '%s: ' ; read )\n", press_enter_to_close);
    else
        keepterm = g_strdup("");
    ...
    if (!ssh_udevil) // !sync
    {
        ...
        ao->keep_point = (run_in_terminal && (terminal = xset_get_s("main_terminal")) &&
                          (strstr(terminal, "lxterminal") || strstr(terminal, "urxvtc") ||
...
static void on_check_root(GtkMenuItem* item, VFSVolume* vol, GtkWidget* view2)
{
    ...
    {
        msg = g_strdup_printf(
            _("%s is currently mounted.  You must unmount it before you can check it."),
            vol->device_file);
        ...
        set->s)
    {
        ...
        g_free(task_name);
        char* scmd = g_strdup_printf("echo \">>> %s\"; echo; %s; echo; echo -n \"%s: \"; read s",
                                     cmd,
                                     cmd,
                                     press_enter_to_close);
        g_free(cmd);
...
static void on_change_label(GtkMenuItem* item, VFSVolume* vol, GtkWidget* view2)
{
    ...
    {
        if (!strcmp(vol->fs_type, "ext2") || !strcmp(vol->fs_type, "ext3") ||
    ...
    if (vol->is_mounted)
    {
        mount_warn = g_strdup_printf(_("\n\nWARNING: %s is mounted.  You may want or need to "
                                       "unmount it before changing the label."),
                                     vol->device_file);
    }
    else
        mount_warn = g_strdup("");
    ...
    if (!new_label)
    {
        ...
        def_cmd = set->desc;
        msg = g_strdup_printf(
            _("Enter remove label command for fstype '%s':\n\nUse:\n\t%%%%v\tdevice file ( %s "
    ...
    else
    {
        ...
        def_cmd = set->title;
        msg = g_strdup_printf(
            _("Enter change label command for fstype '%s':\n\nUse:\n\t%%%%v\tdevice file ( %s "
              ")\n\t%%%%l\tnew label ( \"%s\" )\n\nEDIT WITH CARE   This command is run as root%s"),
            vol->fs_type ? vol->fs_type : "none",
            vol->device_file,
            new_label,
            mount_warn);
    }
...
static void on_format(GtkMenuItem* item, VFSVolume* vol, GtkWidget* view2, XSet* set2)
{
    ...
    {
        msg = g_strdup_printf(
            _("%s is currently mounted.  You must unmount it before you can format it."),
            vol->device_file);
    ...
    {
        msg = g_strdup_printf(_("You are about to erase %s %s- ALL DATA WILL BE LOST.  Be patient "
                                "- this can take awhile and dd gives no feedback.\n\nEnter command "
                                "to overwrite entire volume with /dev/%s:\n\nUse:\n\t%%%%v\tdevice "
                                "file ( %s )\n\nEDIT WITH CARE   This command is run as root"),
                              vol->device_file,
                              entire,
                              set->desc,
        ...
        msg = g_strdup_printf(_("You are about to format %s %s- ALL DATA WILL BE LOST.\n\nEnter %s "
                                "format command:\n\nUse:\n\t%%%%v\tdevice file ( %s )\n\nEDIT WITH "
                                "CARE   This command is run as root"),
                              vol->device_file,
                              entire,
                              set->desc,
                              vol->device_file);
        ...
        set->s)
    {
        ...
        PtkFileTask* task = ptk_file_exec_new(task_name, NULL, view, file_browser->task_view);
        g_free(task_name);
        char* scmd = g_strdup_printf(
            "echo \">>> %s\"; echo; echo \"%s %s\"; echo; echo -n \"%s: \"; read s; if [ \"$s\" != "
            "\"yes\" ]; then exit; fi; %s; echo; echo -n \"%s: \"; read s",
            cmd,
            data_loss_overwrite,
            vol->device_file,
            type_yes_to_proceed,
...
static void on_restore(GtkMenuItem* item, VFSVolume* vol, GtkWidget* view2, XSet* set2)
{
    ...
    {
        msg = g_strdup_printf(
            _("%s is currently mounted.  You must unmount it before you can restore it."),
            vol->device_file);
        xset_msg_dialog(view,
                        GTK_MESSAGE_ERROR,
                        _("Device Is Mounted"),
    ...
    char* bfile = xset_file_dialog(view,
                                   GTK_FILE_CHOOSER_ACTION_OPEN,
                                   _("Choose Backup For Restore"),
                                   set->z,
                                   set->s);
    ...
    else if (g_str_has_suffix(bfile, ".mbr.bin") || g_str_has_suffix(bfile, ".mbr") ||
             g_str_has_suffix(bfile, "-MBR-back"))
    {
        ...
        msg = g_strdup_printf(
            _("You are about to overwrite the MBR boot code of %s using %s.\n\nThis change may "
              "prevent your computer from booting properly.  All important data on the entire "
              "device should be backed up first.\n\nProceed?  (If you press Yes, you still have "
              "one more opportunity to abort before the disk is modified.)"),
            vfile,
            bname);
        if (xset_msg_dialog(view,
                            GTK_MESSAGE_WARNING,
                            _("Restore MBR"),
                            NULL,
                            GTK_BUTTONS_YES_NO,
                            _("DATA LOSS WARNING"),
                            msg,
                            "#devices-root-resfile") != GTK_RESPONSE_YES)
        {
            g_free(vfile);
            g_free(bfile);
            g_free(bname);
    ...
    else
    {
        xset_msg_dialog(view,
                        ...
                        NULL,
                        0,
                        _("The selected file is not a recognized backup file.\n\nFSArchiver "
                          "filenames contain 'fsarchiver' or end in '.fsa'.  Partimage filenames "
                          "contain 'partimage' or end in '.000'.  MBR filenames end in '.mbr', "
                          "'.mbr.bin', or '-MBR-back' and are 512 bytes in size.  If you are SURE "
                          "this file is a valid backup, you can rename it to avoid this error."),
                        NULL,
                        "#devices-root-resinfo");
        g_free(bfile);
        g_free(bname);
        return;
    }
    ...
    else
    {
        ...
        if (strlen(vol->device_file) < 9)
            ...
            entire = "";
        msg = g_strdup_printf(
            _("You are about to restore %s %susing %s - ALL DATA WILL BE LOST.\n\nEnter %s restore "
              "command:\n\nUse:\n\t%%%%v\tdevice file ( %s )\n\t%%%%s\tbackup file ( %s )\n\nEDIT "
              "WITH CARE   This command is run as root"),
            vol->device_file,
            entire,
            bname,
            type,
    ...
    if (job == 2)
        scmd = g_strdup_printf(
            "echo \">>> %s\"; echo; echo \"%s %s\"; echo; echo -n \"%s: \"; read s; if [ \"$s\" != "
            "\"yes\" ]; then exit; fi; %s; echo; echo -n \"%s \"; read s",
            cmd,
            "DATA LOSS WARNING - overwriting MBR boot code of",
            vfile,
            type_yes_to_proceed,
            cmd,
            press_enter_to_close);
    ...
    else
    {
        ...
        scmd = g_strdup_printf(
            "echo \">>> %s archinfo %s\"; echo; %s archinfo %s; echo; echo \">>> %s\"; echo; echo "
            "\"%s %s\"; echo; echo -n \"%s: \"; read s; if [ \"$s\" != \"yes\" ]; then exit; fi; "
            "%s; echo; echo -n \"%s: \"; read s",
            fsarc_bin,
            sfile,
            fsarc_bin,
            sfile,
...
static void on_restore_info(GtkMenuItem* item, GtkWidget* view, XSet* set2)
{
             ...
             g_str_has_suffix(bfile, "-MBR-back"))
    {
        xset_msg_dialog(view,
                        ...
                        NULL,
                        0,
                        _("Based on its name, the selected file appears to be an MBR backup file.  "
                          "No other information is available for this type of backup."),
                        NULL,
                        "#devices-root-resinfo");
        g_free(bfile);
        return;
    ...
    task->task->exec_command =
        g_strdup_printf("echo \">>> %s\"; echo; %s; echo; echo -n \"%s: \"; read s",
                        cmd,
                        cmd,
                        press_enter_to_close);
    g_free(cmd);
...
static void on_backup(GtkMenuItem* item, VFSVolume* vol, GtkWidget* view2, XSet* set2)
{
    ...
    else if (!strcmp(set->name, "dev_back_part"))
    {
        if (vol->is_mounted)
        {
            msg = g_strdup_printf(_("%s is currently mounted.  You must unmount it before you can "
                                    "create a backup using Partimage."),
                                  vol->device_file);
            xset_msg_dialog(view,
                            GTK_MESSAGE_ERROR,
                            _("Device Is Mounted"),
    ...
    if (!strcmp(set->name, "dev_back_part"))
    {
        ...
        if (g_file_test(volb, G_FILE_TEST_EXISTS))
        {
            if (xset_msg_dialog(view,
                                ...
                                NULL,
                                GTK_BUTTONS_YES_NO,
                                _("The selected backup already exists.  Overwrite?"),
                                NULL,
                                "#devices-root-parti") != GTK_RESPONSE_YES)
            {
                g_free(vfile);
                g_free(bfile);
                g_free(volb);
